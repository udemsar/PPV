...
# ------------------------------------------------
# Writes data from the three or four 3D arrays representing xcoordinates, ycoordinates,
...
WriteCSVtableFromThreeRectangular3DArrays <- function(xc,yc,zc,FileName) {
  ...
  # Build an empty CSV table of correct dimensions
  CSVtable <- array(data=0,dim=c(rowsX*colsX*sheetsX,3))
  colnames(CSVtable) <- c('X','Y','Z')
  ...
  for (k in 1:sheetsX) {
    ...
    for (i in 1:rowsX) { 
      for (j in 1:colsX) {
        # print(c(i,j,k))
        CSVtable[(k-1)*rowsX*colsX+(i-1)*colsX+j,] <- c(xc[i,j,k],yc[i,j,k],zc[i,j,k])
      } # j
    } # i
  } # k 
